...
import {
  ...
  Collector,
  NFTsCollectors,
  NFTsCollectorsChanges,
} from '../../types/nftsCollectors';
...
const processorFunction =
  (chainId: ChainId, contracts: NftFactory[], name: string) =>
    async (
      { newCursor, items }: { newCursor: Cursor; items: ethers.Event[] },
      clients: Clients,
    ) => {
      ...
      items.forEach((item): void => {
        ...
        if (!newMint(fromAddress)) {
          ...
          nftsCollectorsChanges.push({
            nftId: nftId,
            collectorId: fromAddress,
            amount: -1,
          });
          if (!burned(toAddress)) {
            nftsCollectorsChanges.push({
              nftId: nftId,
              collectorId: toAddress,
              amount: 1,
            });
          }
        } else {
          newNFTs.push({
            ...
            publicReleaseTime:
            contract.typeMetadata?.other?.publicReleaseTime || undefined,
            chainId,
          });
          nftsCollectorsChanges.push({
            nftId: nftId,
      ...
      const allCollectors = nftsCollectorsChanges.map<Collector>(
        ({ collectorId }) => {
          return { id: collectorId };
        },
      );
      const idPairs = _.uniq(
        nftsCollectorsChanges.map(e => [e.nftId, e.collectorId]),
      );
      const idPairChunks = _.chunk(idPairs, 500);
      const existingNftsCollectors: NFTsCollectors[] = [];
      for (let i = 0; i < idPairChunks.length; i++) {
        const records = await clients.db.getRecords<NFTsCollectors>(
          Table.nftsCollectors,
          [['whereIn', [['nftId', 'collectorId'], idPairChunks[i]]]],
        );
        existingNftsCollectors.push(...records);
      }

      const allNftsCollectorsChanges = nftsCollectorsChanges.concat(
        existingNftsCollectors,
      );
      ...
      for (let i = 0; i < updatedNftsCollectorsChunks.length; i++) {
        await clients.db.upsert(
          Table.nftsCollectors,
          updatedNftsCollectorsChunks[i],
          ['nftId', 'collectorId'],
        );
      }
      ...
      const existingNfts = new Set(
        (
          await clients.db.getRecords<NFT>(Table.nfts, [
            ['whereIn', ['id', transferNftIds]],
          ])
        ).map(nft => nft.id),
      );
      const transfersForExistingNfts = transfers.filter(transfer =>
        existingNfts.has(transfer.nftId!),
      );
...
export const createERC721NFTsFromTransfersProcessor: (
  chainId: ChainId,
  contracts: NftFactory[],
) => Processor = (chainId, contracts) => {
    ...
    trigger: newERC721Transfers(
      ...
      contracts
        ...
        .map(c => ({
          ...
          address: c.address,
        })), // map contracts to EthereumContract
