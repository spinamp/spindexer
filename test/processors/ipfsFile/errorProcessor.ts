...
import { ipfsFileErrorRetry, ipfsFileUrlErrorRetry } from '../../../src/processors/ipfsFile/errorProcessor';
...
describe('ipfsFileErrorRetry', async () => {
  ...
  describe('trigger', async () => {
    ...
    it ('returns ipfsFiles with errors', async () => {
      ...
      assert(result[1].cid === '2xx', `incorrect row returned, result was ${JSON.stringify(result[1])}`);
    });
  })
...
describe('ipfsFileUrlErrorRetry', async () => {
  let dbClient: DBClient;
  let clients: Clients;
  ...
  const ipfsFilesUrls: IPFSFileUrl[] = [
    ...
    { url: 'https://spinamp.xyz/5xx', error: 'nope', cid: '5xx' }, // skipped when cid not null
  ]
  ...
  describe('trigger', async () => {
    ...
    it ('returns ipfsFileUrls with errors', async () => {
      const result: any = await ipfsFileUrlErrorRetry.trigger(clients, undefined);

      assert(result.length === 2, `should only return 1 file based on test data, instead returned ids: ${ result.length > 0 ? result.map((t: any) => t.cid) : 'none' }`);
      assert(result[0].url === 'https://spinamp.xyz/1xx', `incorrect row returned, result was ${JSON.stringify(result[0])}`);
      assert(result[1].url === 'https://spinamp.xyz/2xx', `incorrect row returned, result was ${JSON.stringify(result[1])}`);
    });
  })
  ...
  describe('processor', async () => {
    ...
    it('clears the error, increments the retry, and sets lastRetry', async () => {
      const triggerItems = await ipfsFileUrlErrorRetry.trigger(clients, undefined);
      await ipfsFileUrlErrorRetry.processorFunction(triggerItems, clients);

      const files: any = await dbClient.getRecords(Table.ipfsFilesUrls, [['where', ['url', 'like', '%1xx%']]]);
